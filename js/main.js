...
function getState(k){ return JSON.parse(localStorage.getItem(k)||'{"guesses":[],"count":0,"finished":false,"won":false}'); }
...
document.addEventListener("DOMContentLoaded",()=>{
  const idx=getTodayIndex(), answer=playlist[idx].title, key=`state-${idx}`;
  ...
  function renderSegments(){
    ...
    durations.forEach((sec,i)=>{
      ...
      div.style.flex=sec/total; div.style.height="100%";
  ...
  function playStage(){
    ...
    playDiv.style.position="absolute"; playDiv.style.top="0"; playDiv.style.left="0";
    playDiv.style.height="100%"; playDiv.style.backgroundColor="#34D399";
    playDiv.style.width="0%"; playDiv.style.transition=`width ${sec}s linear`;
    el.progressSegments.appendChild(playDiv);

    requestAnimationFrame(()=>playDiv.style.width=`${(sec/total)*100}%`);

    setTimeout(()=>{
      el.audio.pause(); el.audio.currentTime=0; renderSegments();
    }, sec*1000);
  }
  ...
  function placeGuesses(){
    ...
    state.guesses.slice(0, maxGuesses).forEach(txt=>{
      ...
      d.style.width = w; d.textContent = txt;
      el.guessGrid.appendChild(d);
    });
    for(let i=state.guesses.length; i<maxGuesses; i++){
      const d=document.createElement("div");
      d.className="h-12 bg-gray-800 rounded flex items-center justify-center text-sm text-white mb-2";
      d.style.width = w; el.guessGrid.appendChild(d);
    }
  }
  ...
  function updateStats(won){
    ...
    localStorage.setItem("games", g); el.gamesPlayed.textContent = g;
    let s = Number(localStorage.getItem("streak") || 0);
    if(won){ s++; localStorage.setItem("streak", s); 
      const b = Number(localStorage.getItem("best") || 0); if(s > b) localStorage.setItem("best", s);
    } else {
  ...
  function showAutocomplete(){
    ...
    playlist.forEach(song=>{
      if(song.title.toLowerCase().includes(q)){
        ...
        d.onclick=()=>{ el.guessInput.value = song.title; el.acList.innerHTML=""; el.acList.classList.add("hidden"); };
  ...
  el.submitBtn.addEventListener("click", ()=>{ const v=el.guessInput.value.trim(); if(v) handleGuess(v); });
