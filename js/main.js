...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  function playStage(){
    ...
    const total=durations[durations.length-1];
    el.audio.currentTime=0; el.audio.play();
    renderSegments();

    const playDiv=document.createElement("div");
    playDiv.style.position="absolute";
    playDiv.style.height="100%";
    playDiv.style.left="0"; playDiv.style.top="0";
  ...
  function updateStats(won){
    ...
    } else { s=0; localStorage.setItem("streak","0"); }
  ...
  function showAutocomplete(){
    ...
    playlist.forEach(it=>{
      if(it.title.toLowerCase().includes(q)){
        ...
        d.onclick=()=>{el.guessInput.value=it.title;el.acList.innerHTML="";};
  ...
  if(state.finished){
    disableGame(el);
    el.correctAnswer.textContent=playlist[idx].title; el.correctAnswer.classList.remove("hidden");
  }
