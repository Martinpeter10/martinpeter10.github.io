...
async function fetchPlaylist() {
  const r = await fetch(`https://api.soundcloud.com/resolve?url=${encodeURIComponent(PLAYLIST_URL)}&client_id=${CLIENT_ID}`);
  if (!r.ok) throw new Error('Playlist failed');
  const data = await r.json();
  const tracks = data.tracks || [];
  if (tracks.length < titles.length) throw new Error('Not enough tracks');
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    console.error(err);
    return alert('Playlist load error. Check CLIENT_ID and playlist settings');
  }
  ...
  let count = 0, guessed = new Set(), max = 6;
  updateStats(); updateCountdown(); setInterval(updateCountdown,1000);
  ...
  function updateCountdown(){
    ...
    m.setHours(24,0,0,0);
    const d=m-n, h=Math.floor(d/3600000), mm=Math.floor((d%3600000)/60000), s=Math.floor((d%60000)/1000);
    el.countdown.textContent = `⏳ New theme song in ${h}h ${mm}m ${s}s`;
  ...
  function handleGuess(){
    ...
    d.textContent=`${ok?'✅':'❌'} ${g} @ ${t}s`;
    d.className = ok?'text-green-300 font-bold':'text-red-300';
    el.history.appendChild(d);
    el.guessInput.value=''; count++; el.currentGuess.textContent = Math.min(count+1, max);
  ...
  function handleSkip(){
    ...
    const t=[1,2,3,5,10,15][Math.min(count,5)];
    const d=document.createElement('div');
    d.textContent = `⏭️ Skipped @ ${t}s`; d.className='text-yellow-200';
    el.history.appendChild(d); count++;
  ...
  function finish(correct){
    ...
    let cs = parseInt(localStorage.getItem('currentStreak')||0), bs=parseInt(localStorage.getItem('bestStreak')||0);
  ...
  function autoComp(){
    ...
    playlist.filter(t=>t.title.toLowerCase().includes(v)).forEach(t=>{
      ...
      div.textContent=t.title; div.className='cursor-pointer px-3 py-1 hover:bg-gray-200';
